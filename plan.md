...
| 6b | `digest()` not in search path → audit hash chain broken | After enabling server-side audit triggers, every UPDATE on workers/sites/projects failed with `42883 function digest(text, unknown) does not exist`. The original `audit_log_hash_chain` (0003) used `pgcrypto.digest()`, but pgcrypto lives in `extensions` schema, not `public`, so the trigger couldn't resolve it. | Replaced with built-in `sha256(bytea)` (Postgres 11+, no extension dependency). End-to-end re-verified: supervisor UPDATE succeeds, audit row written with 64-char hex hash. | ✅ Fixed in [`0010_fix_audit_hash.sql`](supabase/migrations/0010_fix_audit_hash.sql) |
